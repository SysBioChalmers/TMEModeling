...
constructEquations(mx, 'total_biomass') %{'biomass[c] + 0.016835 ECM_biomass[f_s] => '} 
%the stochiomentry looks reasonable
constructEquations(mx, 'ECM_biomass') %{'0.063828 heparan sulfate proteoglycan[f_s] + 8.4379 ECM_protein_pool_biomass[f_s] => ECM_biomass[f_s]'}
...
%check that communication from other cells is blocked - use L-lactate as example
scomp = find(strcmp(mx.comps,'s'));
...
%    {'glucose[s] => glucose[c]'                                                                                           }
%    {' => glucose[s]'                                                                                                     }
%    {'1e-06 prot_pool[c] + glucose[f_s] => glucose[s]'                                                                    }
%    {'glucose[s] => glucose[f_s]'                                                                                         }
%    {'glucose[s] => glucose[o_s]'                                                                                         }
...
%% TC003 - 
load('data/ltModel.mat');
modelTmp = constrainMedium(ltModel, {'glucose[s]'}, [7 13], false, true);
...
constructEquations(modelTmp2, modelTmp2.rxns(exchIndxsFilt2)) %in total 91
%    {' => H2O[s]'    }
%    {' => Pi[s]'     }
%    {' => sulfate[s]'}
%    {' => Fe2+[s]'   }
...
%    {'MAR05029_REV'}        3.5643         {'glucose[s] => glucose[c]'                                                                }
