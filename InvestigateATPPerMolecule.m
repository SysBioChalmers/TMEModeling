...
listMetRxnsWithFluxes(baseModelSuccExp, sols_c1{3}, 'succinate', true, 10^-4); %succinate is exported - this is 55% of the glutamine
listMetRxnsWithFluxes(baseModelSuccExp, sols_c1{3}, 'ornithine', true, 10^-4); %is eventually exported
listMetRxnsWithFluxes(baseModelSuccExp, sols_c1{3}, 'citrulline', true, 10^-4); %just a cycle for exporting ornithine
...
m.ub(strcmp(mBase.rxns, 'succExp')) = 0;%so, the prodh reaction is actually defined in reverse in the model
tic
D5_6_1 = runASimulation(m, a, bloodData, 0, false, NaN, 'MAR03964'); 
toc
save('data/D5_6_1.mat', 'D1_1')
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%Now investigate the role of dihydroorotate.
