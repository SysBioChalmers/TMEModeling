...
library(ggplot2)
library(reshape2)
setwd("C:/Work/MatlabCode/projects/HumanGemZenodo/Human1_Publication_Data_Scripts/ec_GEMs/ComplementaryScripts")
...
x = rep(0, length(samples))
x[startsWith(samples,'exp_')] = 1
x[startsWith(samples,'lt_')] = 2

sample.types <- factor(x, labels=c('Pred. GECKO','Measured', 'Pred. GeckoLight'))

# rearrange data
fluxdata <- NULL
for (i in 1:(ncol(fluxes)/3)) {
  add_data <- fluxes[, c(1, (3*i):(3*i+2))]
  add_data$cell.line <- colnames(fluxes)[3*i+1]
  colnames(add_data) <- c('Metabolite','Measured','Pred. GECKO','Pred. GeckoLight','Cell.Line')
